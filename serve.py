...
"""Local preview. Static files, correct MIME types, byte ranges.
    ...
    python3 serve.py           # http://localhost:8211, nothing cached
    python3 serve.py --pages   # send GitHub Pages' own Cache-Control (max-age=600)
                               # to rehearse a redeploy against the service worker

Add ?nosw to the URL to run without the offline worker.
"""
import functools
import http.server
import os
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def copyfile(self, source, outputfile):
        try:
            ...
            pass  # the browser let go of a video range it no longer needs
...
if __name__ == "__main__":
    with Server(("", PORT), functools.partial(Handler, directory=str(ROOT))) as srv:
        print(f"Spa Sahel → http://localhost:{PORT}" + ("  (GitHub Pages cache headers)" if PAGES else ""))
        srv.serve_forever()
